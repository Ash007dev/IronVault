"""
IronVault Authentication Routes
=================================
Handles user registration, login with MFA, and password reset.

Endpoints:
- POST /auth/register - Create new account
- POST /auth/login - Password verification → OTP sent via Email
...
def register():
    ...
    role = data.get('role', 'operative').lower()
    email = data.get('email').strip()
    
    if role not in ['operative', 'co']:
        return jsonify({'error': 'Role must be operative or co'}), 400
...
def reset_password():
    ...
    Supports both email-based and username-based reset.
    """
    data = request.get_json()
    
    # Support both email-based and username-based reset
    if 'email' in data:
    ...
    if not data.get('otp') or not data.get('new_password'):
...
# =============================================================================
# EMAIL-BASED PASSWORD RESET (matches frontend API)
# =============================================================================

@auth_bp.route('/request-password-reset', methods=['POST'])
def request_password_reset():
    """Request password reset via Email - uses email to find user."""
    data = request.get_json()
    ...
    if user:
        otp = generate_otp()
        create_otp(user['username'], otp)
        send_otp_via_email(user['username'], email, otp)
        
        create_audit_log(
            user_id=user['id'],
            username=user['username'],
            action='PASSWORD_RESET_REQUESTED',
            details=f'Reset OTP sent to {masked_email}',
            ip_address=get_client_ip()
        )
    
    return jsonify({
        'success': True,
        'message': f'If an account with this email exists, a reset code has been sent to {masked_email}'
    })
...
def verify_reset_otp():
    ...
    if not verify_otp(user['username'], otp_code, consume=False):
...
def webauthn_register_verify():
    ...
    if not challenge:
        return jsonify({'error': 'Challenge expired or invalid. Please try again.'}), 400
    ...
    print(f"[WebAuthn] Received registration data keys: {list(data.keys())}")
...
@auth_bp.route('/webauthn/check', methods=['POST'])
def webauthn_check():
    ...
    if not username:
        return jsonify({'has_passkey': False}), 200
    ...
    credentials = get_webauthn_credentials(user['id'])
    return jsonify({'has_passkey': len(credentials) > 0}), 200
