"""
IronVault Cryptographic Utilities
==================================
...
def encrypt_file_with_signature(file_data):
    """
    Encrypt file data and sign it.
    Format: Base64( IV[16 bytes] + Signature[256 bytes] + Ciphertext )
    
    This is the secure token format for encrypted documents.
    
    Args:
        file_data (bytes): Raw file bytes
        
    Returns:
        tuple: (encrypted_token, token_info_dict)
    """
    ...
    # Sign the ciphertext (RSA-PSS signature is 256 bytes for RSA-2048)
    ...
    # Combine: IV[16] + Signature[256] + Ciphertext
    combined = iv + signature + ciphertext
...
def decrypt_file_with_signature(encrypted_token):
    """
    Decrypt and verify a signed encrypted file.
    
    Args:
        encrypted_token (str): Base64 encoded encrypted token
        
    Returns:
        tuple: (success: bool, file_data or error_message, signature_valid: bool)
    """
    try:
        # Decode the token
        combined = base64.b64decode(encrypted_token)
        
        # Extract parts: IV[16] + Signature[256] + Ciphertext
        iv = combined[:16]
        signature = combined[16:272]  # 256 bytes
        ...
        # Decrypt even if signature is invalid (to show the data)
        cipher = Cipher(algorithms.AES(AES_KEY), modes.CBC(iv), backend=default_backend())
        decryptor = cipher.decryptor()
        padded_data = decryptor.update(ciphertext) + decryptor.finalize()
        ...
        return True, file_data, signature_valid
...
def encrypt_data(plaintext):
    """
    Encrypt data and return combined IV:ciphertext string.
    Wrapper for easier API usage.
    """
    ciphertext_b64, iv_b64 = aes_encrypt(plaintext)
    return f"{iv_b64}:{ciphertext_b64}"


def decrypt_data(encrypted_string):
    """
    Decrypt data from combined IV:ciphertext string.
    Wrapper for easier API usage.
    """
    iv_b64, ciphertext_b64 = encrypted_string.split(':')
    return aes_decrypt(ciphertext_b64, iv_b64)


def generate_token(data):
    """
    Generate a verification token by signing the data.
    Used for integrity verification.
    
    Args:
        data (str): Data to create token for
        
    Returns:
        str: Base64 encoded signature token
    """
