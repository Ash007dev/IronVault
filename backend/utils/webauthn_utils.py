...
from config import RP_ID, RP_NAME, WEBAUTHN_ORIGINS
...
def get_origin():
    ...
    if origin and origin in WEBAUTHN_ORIGINS:
        return origin
    # Check referer as fallback
    referer = request.headers.get('Referer', '')
    for allowed_origin in WEBAUTHN_ORIGINS:
        if referer.startswith(allowed_origin):
            return allowed_origin
    # Default to first allowed origin
    return WEBAUTHN_ORIGINS[0]
...
def verify_reg_response(credential_dict, challenge_bytes):
    ...
    try:
        ...
        print(f"[WebAuthn] Verification error: {str(e)}")
        print(f"[WebAuthn] Credential dict keys: {list(credential_dict.keys()) if credential_dict else 'None'}")
        raise
